...
public class SMSCSender {
    ...
    private String SMSC_LOGIN    = "<login>";     // логін клієнта
    private String SMSC_PASSWORD = "<password>";  // пароль або  MD5-хеш пароля (нижній регістр)
    private boolean SMSC_HTTPS   = false;         // використання HTTPS протоколу
    private String SMSC_CHARSET  = "utf-8";       // кодування повідомлення: koi8-r, windows-1251 или utf-8 (по замовчуванню)
    private boolean SMSC_DEBUG   = false;         // debugging
    private boolean SMSC_POST    = false;         // икористовувати метод POST
    ...
    public String[] sendSms(String phones, String message, int translit, String time, String id, int format, String sender, String query) {
        ...
        if (SMSC_DEBUG) {
            if (Integer.parseInt(m[1]) > 0) {
                System.out.println("Повідомлення успішно відправленно. ID: " + m[0] + ", всего SMS: " + m[1] + ", вартість: " + m[2] + ", баланс: " + m[3]);
            }
            else {
    ...
    public String[] getSmsCost(String phones, String message, int translit, int format, String sender, String query){
        ...
        if (SMSC_DEBUG) {
            if (Integer.parseInt(m[1]) > 0) {
                System.out.println("Вартість розсилки: " + m[0] + ", Всього SMS: " + m[1]);
            }
            else {
                System.out.print("Помилка №" + Math.abs(Integer.parseInt(m[1])));
            }
        }
    ...
    public String getBalance() {
        ...
        if (SMSC_DEBUG) {
            if (m.length == 1)
                System.out.println("Сума на рахунку: " + m[0]);
            else
                System.out.println("Помилка №" + Math.abs(Integer.parseInt(m[1])));
        }
    ...
    private String SmscReadUrl(String url) {
        ...
        catch (MalformedURLException e) { // Не вірний url, протокол...
